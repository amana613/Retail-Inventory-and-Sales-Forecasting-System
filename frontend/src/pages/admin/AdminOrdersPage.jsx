...
const AdminOrdersPage = () => {
  ...
  if (loading) return <div style={{ minHeight: '50vh', display: 'flex', alignItems: 'center' }}><Loader text="Loading order data..." /></div>;
          ...
          <tbody>
            {orders.map((order) => (
              <tr key={order._id} style={{ borderBottom: '1px solid #f1f5f9' }}>
                <td style={{ fontWeight: '500' }}>#{order._id.substring(0, 8)}</td>
                <td>
                  <div style={{ display: 'flex', flexDirection: 'column' }}>
                    <span style={{ fontWeight: '500', color: 'var(--text-dark)' }}>{order.user && order.user.name}</span>
                  </div>
                </td>
                <td style={{ color: 'var(--text-muted)' }}>{new Date(order.createdAt).toLocaleDateString()}</td>
                <td style={{ fontWeight: '600' }}>Rs. {order.totalPrice.toFixed(2)}</td>
                  ...
                  {order.isPaid ? (
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', alignItems: 'flex-start' }}>
                      ...
                      <div style={{ display: 'flex', gap: '6px' }}>
                        {order.receiptImage ? (
                          <button 
                            onClick={() => handleViewReceipt(order.receiptImage)}
                            style={{ display: 'flex', alignItems: 'center', gap: '4px', padding: '4px 8px', fontSize: '0.75rem', background: '#f8fafc', color: '#475569', border: '1px solid #cbd5e1', borderRadius: '4px', cursor: 'pointer' }}
                        ...
                        <button 
                          ...
                          style={{ padding: '4px 8px', fontSize: '0.75rem', background: '#2563eb', color: 'white', border: 'none', borderRadius: '4px', cursor: 'pointer' }}
                        >
                          Verify & Mark Paid
                        </button>
                      </div>
                ...
                <td>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', alignItems: 'flex-start' }}>
                    ...
                    <div style={{ display: 'flex', gap: '6px' }}>
                      ...
                      <select 
                        value={order.rider ? (order.rider._id || order.rider) : ''} 
                        onChange={(e) => handleRiderAssignment(order._id, e.target.value)}
                ...
                <td>
                  <div className="table-actions">
                    <button className="btn-icon" onClick={() => navigate(`/order/${order._id}`)} title="View Order Details" style={{ background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: '6px', padding: '6px' }}>
            ...
            {orders.length === 0 && (
              <tr>
                <td colSpan="7" style={{ textAlign: 'center', padding: '4rem 2rem' }}>
                  <Message variant="info">No orders found matching current criteria.</Message>
                </td>
              </tr>
            )}
          </tbody>
        ...
        <div style={{ display: 'flex', justifyContent: 'center', padding: '10px' }}>
          {selectedReceipt ? (
            <img 
              src={selectedReceipt} 
              alt="Payment Receipt" 
              style={{ maxWidth: '100%', maxHeight: '600px', borderRadius: '8px', objectFit: 'contain', border: '1px solid #f1f5f9' }} 
            />
          ) : (
